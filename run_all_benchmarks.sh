...
setup() {
	...
	echo "Algorithm, File_Size, Encrypted_Size, Resident_Size, User_Time, System_Time, Wall_Clock_Time, Time_Nanoseconds, Compression_Algo" > "$DATA_FILE"
...
record_data() {
	...
	echo "$alg_name, $data_file_size, $encrypted_size, $resident_size, $user_time, $system_time, $wall_clock_time", $time_nano , $compression_algo >> "$DATA_FILE"
}
...
run_encryption_on_algorithm() {
    ...
    for data_filename in $INPUT_DATA_DIR/*; do
	    ...
	    record_data "$alg_name" "$file_size" "$encrypted_size" "$RESIDENT_SIZE" "$USER_TIME" "$SYSTEM_TIME" "$WALL_CLOCK_TIME" "$TIME_NANO" "$val"
	    done
